...
def ensure_preview_script(workspace: Path) -> Path:
    source = TOOL_TEMPLATE_DIR / "scripts" / "preview-official-questions.mjs"
    if not source.exists():
        raise FileNotFoundError(f"未找到预览脚本：{source}")
...
def main() -> int:
    ...
    if not args.preview_all and args.limit <= 0:
        args.limit = 10
    ...
    try:
        preview_script = ensure_preview_script(workspace)
